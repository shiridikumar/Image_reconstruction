...
class Imputation:
    ...
    def synthetic_mask(self, fraction_to_keep = 0.7,corrupted_img=None,mask=None):
        '''
            def: Generates a synthetic mask for the image.
        '''
        if(corrupted_img):
            self.set_corrupt_img(corrupted_img,mask)
    ...
    def impute(self, speed = "fast", hil = 0, skip_vis=0, hil_param = 25):
        '''
            def: Imputes missing data in a given dataframe.
            @args speed: "slow"/"fast"(default), speed of the algorithm
            @args hil: 0-> No CMTF4SI, 1-> Include CMTF4SI, 2-> Only CMTF4SI
